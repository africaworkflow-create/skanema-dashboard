...
const STEPS = [
  ...
  {
    id     : 'whatsapp',
    label  : 'Activation WhatsApp par Skanema',
    desc   : 'Notre équipe configure votre bot sous 24h. Contactez-nous pour accélérer.',
    done   : false,
    link   : null,
...
export function SetupChecklist() {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden mb-6 shadow-sm">
                  ...
                  {!step.done && step.cta && (
                    step.external ? (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-medium bg-gray-900
                                   text-white px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors"
                      >
                        {step.cta}
                      </a>
                    ) : (
      ...
      {/* Footer si tout est done */}
      {allDone && (
        <div className="px-5 py-4 bg-green-50 border-t border-green-100 flex items-center justify-between">
          <p className="text-xs text-green-700 font-medium">
            🚀 Votre restaurant est prêt à recevoir des commandes !
          </p>
          <button onClick={handleDismiss} className="text-xs text-green-600 underline">
            Fermer
          </button>
        </div>
      )}
