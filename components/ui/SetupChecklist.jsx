...
const STEPS = [
  ...
  { id: 'menu',     label: 'Ajoutez votre premier plat',     desc: 'Commencez par ajouter vos plats avec photos et prix.',                              done: false, link: '/dashboard/menu',     cta: 'Ajouter un plat →' },
...
export function SetupChecklist() {
  ...
  useEffect(() => {
    ...
    check()
  }, [mounted, authLoading, user, dismissed])
      ...
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900">{allDone ? '🎉 Tout est prêt !' : 'Démarrer avec Skanema'}</p>
              <p className="text-xs text-gray-400 mt-0.5">{doneCount}/{steps.length} étapes complétées</p>
            </div>
          </div>
          <button onClick={handleDismiss} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors flex-shrink-0">
            <X size={14} />
          </button>
        </div>
            ...
            <div key={step.id} className={`border-b border-gray-50 last:border-0 ${step.done ? 'opacity-60' : ''}`}>
              <button onClick={() => setExpanded(isExp ? null : step.id)}
                ...
                <div className="flex-shrink-0">
                  {step.done ? <CheckCircle2 size={20} className="text-green-500" /> : <Circle size={20} className="text-gray-200" />}
                </div>
              ...
              {isExp && (
                <div className="px-5 pb-4 pt-1 ml-8">
                  ...
                  {!step.done && step.cta && (
                    <a href={step.link} className="inline-flex items-center gap-1.5 text-xs font-medium bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors">
                      {step.cta}
                    </a>
                  )}
                  {step.done && (
