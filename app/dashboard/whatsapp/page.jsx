...
export default function WhatsAppPage() {
  ...
  useEffect(() => {
    ...
    // Écoute le message de retour du flow Embedded Signup
  ...
  const handleSignupComplete = async (code, wabaId, phoneNumberId) => {
    ...
    try {
      ...
      setSuccess(true)
      setConnecting(false)
      // Reload pour mettre à jour le statut
    ...
    window.FB.login(
      (response) => {
        if (response.authResponse?.code) {
          // Le message event va gérer la suite
        } else {
          setConnecting(false)
      ...
      {
        config_id           : '', // optionnel — laisser vide pour le flow par défaut
        response_type       : 'code',
  ...
  return (
    <DashboardLayout title="WhatsApp" subtitle="Connexion de votre bot WhatsApp">
      <div className="max-w-2xl space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-900">
                {isActive ? 'Bot actif' : 'Bot non connecté'}
              </p>
              <p className="text-xs text-gray-400 mt-0.5">
                {isActive
                  ? `Numéro ID : ${pid}`
                  : 'Connectez votre numéro WhatsApp Business pour activer le bot'
                }
              </p>
            </div>
          </div>
        ...
        {/* Connexion Embedded Signup */}
        {!isActive && (
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-2">Connecter WhatsApp</h2>
            <p className="text-xs text-gray-500 leading-relaxed mb-5">
              Connectez votre numéro WhatsApp Business en quelques clics. Vous aurez besoin d'un compte Facebook et d'un numéro de téléphone dédié à votre restaurant.
            </p>

            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-5">
              <p className="text-xs text-blue-700 font-medium mb-1">Avant de commencer</p>
              <p className="text-xs text-blue-600 leading-relaxed">
                Le numéro que vous allez connecter ne doit pas être utilisé sur WhatsApp personnel ou WhatsApp Business app. Il sera exclusivement dédié à votre bot Skanema.
              </p>
            </div>
            ...
            {success ? (
              <div className="flex items-center gap-2 text-green-600">
                ...
                <p className="text-sm font-medium">Bot activé avec succès ! Rechargement en cours…</p>
              </div>
            ) : (
              <button
              ...
              >
                ...
                {!fbLoaded && <Loader2 size={14} className="animate-spin" />}
        ...
        {/* Si déjà actif — option reconnexion */}
        {isActive && (
          <div className="bg-white border border-gray-100 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-2">Changer de numéro</h2>
            <p className="text-xs text-gray-500 leading-relaxed mb-4">
              Vous pouvez reconnecter un autre numéro WhatsApp Business si nécessaire.
            </p>
            {error && (
              <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-4">
                <p className="text-xs text-red-600">{error}</p>
        ...
        <div className="bg-gray-50 border border-gray-100 rounded-xl p-5">
          ...
          <div className="space-y-2">
            {[
              'Cliquez sur "Connecter mon WhatsApp Business"',
              'Connectez-vous avec votre compte Facebook professionnel',
              'Sélectionnez ou créez un compte WhatsApp Business',
              'Ajoutez et vérifiez votre numéro de téléphone',
              'Votre bot Skanema est activé automatiquement',
            ].map((step, i) => (
